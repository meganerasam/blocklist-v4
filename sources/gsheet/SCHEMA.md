...
| H | omit-from-whitelist | EXACT host, three consumers (holds the protected search hosts + gamed-vote vetoes): ① step-2 veto on the user whitelist · ② veto on download-sites · ③ since 2026-09-09, the veto on BLANKET upstream allows (`scrubGVetoAllows`, guarded) — path/type-scoped exceptions stay |
...
| J | download-sites | added 2026-09-08 (live, 50 rows), dual role: ① curation-set member — subtracted from every blocking source by curate.php; ② source — normalized (invalid rows warn) − omit-from-whitelist → `sanitized/download-sites.txt` (ABP allow list, exact 7-option template) → DNR allow lane at priority 2 (guards: $document/$~third-party/non-@@ = fail) |
...
Sheet E is declared but not yet created in the workbook — export_url TBD.
