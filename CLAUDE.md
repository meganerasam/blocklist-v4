...
array — Sheet A ∪ kadhosts − ledger dead − omit-from-blocklist − curation set, 42,116; staged from the SAME
...
folder per consumer instead of three): default.json (Sheet C − omit-from-whitelist − omit-from-blocklist, 173) · community.json
...
download-sites effects. Chain re-verified: compile 10,216 rules · budgets fine · shadow gate
...
Matched domain + all subdomains. `export_url` still TBD — ingest skips it, curate/compile
treat the absent mirror as an empty set, so the chain stays green until the sheet exists.
Verified with a stub ({ads.youtube.com, outlinediabase.com}): appends 23 → 21, both domains
at 0 rules / 0 raw occurrences in rules.json, doubleclick.net untouched (no over-reach).
Sheet D itself is UNCHANGED — its 23 domains are the legacy `$blockdom` array carried over
from the production server stopads.php, not something v4 invented.
ALSO 2026-09-10: the letter-named PHP variables ($G/$H/$A/$C/$D/$F/$I) are gone — curate and
compile now use semantic names ($omitWhitelist, $omitBlocklist, $defaultBlocklist …) so a
future re-lettering can never silently shift a variable onto another sheet's data.
